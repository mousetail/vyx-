...
if True:
    ...
    with open("post_ids.txt", "a") as f:
        items = True
        while items:
            response = requests.get("https://api.stackexchange.com/2.3/search/excerpts", params={
                "key": api_key,
                "q": "vyxal is:answer",
                "site": "codegolf",
                "page": str(page)
...
with open("post_ids.txt") as f:
    ...
    for i in range((len(ids)+page_size-1)//page_size):
        res = requests.get("https://api.stackexchange.com/2.3/answers/"+";".join(str(i) for i in ids[i*page_size:i*page_size+page_size]), params={
            "key": api_key,
            "site": "codegolf",
            "filter": post_filter
        })
        ...
        for post in data["items"]:
            ...
            code_elements = [
                *parsed_content.select('pre code'), *parsed_content.select('code')]
